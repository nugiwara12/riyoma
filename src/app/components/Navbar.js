    ...
    <main>
      <nav className="flex justify-between px-4 lg:px-8 items-center py-4 bg-slate-200">
        <div className="flex items-center gap-4">
          ...
          {navlinks.map((d, i) => (
            <div key={i} className="relative">
              ...
              {d.icon && (
                <d.icon
                    className={`absolute top-1 left-16 transform w-5 h-4 ${activeDropdown === i && "rotate-90"} transition-all duration-200`}
                />
              )}
        ...
        <div className="relative">
          <section className="flex items-center gap-1 cursor-pointer " onClick={handleAdminClick}>
            {/* Icon section */}
            ...
            {/* avtar img */}
            {/* Dropdown icon */}
          <IoMdArrowDropright className={`transform w-4 h-5 ${activeDropdown === 'admin' && "rotate-90"} transition-all duration-200`} />
          </section>
          {/* Dropdown */}
          {activeDropdown === 'admin' && (
            <div className="absolute top-full right-0 mt-4 w-[120px] text-center bg-white border border-gray-200 rounded shadow-lg">
              {/* Dropdown content */}
              <ul>
                <li className="px-4 py-2 hover:bg-gray-400 cursor-pointer hover:text-sky-700">Profile</li>
                <li className="px-4 py-2 hover:bg-gray-400 cursor-pointer hover:text-sky-700">About</li>
                <Link href="./auth">
                  <li className="px-4 py-2 hover:bg-gray-400 cursor-pointer hover:text-sky-700">Log out</li>
                </Link>
              </ul>
            </div>
          )}
